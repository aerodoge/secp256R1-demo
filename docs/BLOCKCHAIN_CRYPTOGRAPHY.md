...
SHA-256 一次计算:
- 64 轮迭代
- 每轮: 6次32位加法 + 4次逻辑运算 + 2次移位
- CPU 需要数百个时钟周期

ECDSA 签名:
- 大数模乘、模逆运算
- 256位整数运算，CPU 原生只支持 64 位
- 需要多次循环模拟
...
#### Intel SHA Extensions (x86)

```c
// CPU 指令集扩展 (2016年起支持)
// 专用指令: SHA256RNDS2, SHA256MSG1, SHA256MSG2

#include <immintrin.h>

void sha256_hardware(const uint8_t* data, uint8_t* hash) {
    __m128i state0, state1;
    __m128i msg, tmp;

    // 使用 SIMD 寄存器存储状态
    state0 = _mm_loadu_si128((__m128i*)&H[0]);
    state1 = _mm_loadu_si128((__m128i*)&H[4]);

    // SHA256RNDS2: 单条指令执行 2 轮 SHA-256
    // 原本需要 ~50 条指令，现在 1 条
    state1 = _mm_sha256rnds2_epu32(state1, state0, msg);

    // SHA256MSG1/MSG2: 消息扩展
    msg = _mm_sha256msg1_epu32(msg, tmp);
}

// 性能对比:
// 软件实现: ~15 cycles/byte
// 硬件加速: ~3 cycles/byte (5倍提升)
```

#### ARM Cryptography Extensions

```c
// ARMv8 Crypto Extensions (iPhone 5S+, 2013年起)
#include <arm_neon.h>

void sha256_arm(const uint8_t* data, uint8_t* hash) {
    uint32x4_t state0, state1;
    uint32x4_t msg0, msg1, msg2, msg3;

    // SHA256H: 执行 4 轮 SHA-256
    state0 = vsha256hq_u32(state0, state1, msg0);
...
- CPU (软件): ~50 MH/s
- GPU: ~1 GH/s
- ASIC: 110,000 GH/s (比 CPU 快 200万倍)
```

### 11.3 椭圆曲线硬件加速

#### secp256r1 (P-256) 硬件支持

```
为什么 secp256r1 有广泛硬件支持:
- NIST 标准曲线
- TLS/SSL 大量使用
- 芯片厂商优先实现
```
...
| 硬件                   | 支持的曲线     | 用途               |
|----------------------|-----------|------------------|
| Apple Secure Enclave | secp256r1 | Touch ID/Face ID |
| Intel QAT            | secp256r1 | TLS 加速           |
| ARM CryptoCell       | secp256r1 | 移动设备             |
| TPM 2.0              | secp256r1 | 可信计算             |
| YubiKey              | secp256r1 | 硬件密钥             |
| Google Titan         | secp256r1 | 安全芯片             |

#### Apple Secure Enclave 架构
...
│ 想用 Secure Enclave?                     │
│                                         │
│ secp256k1 ← BTC/ETH 使用                 │
│      ↓                                  │
│ 不支持! Secure Enclave 只支持 P-256       │
│      ↓                                  │
│ 只能软件实现，安全性较低                    │
└─────────────────────────────────────────┘

这就是 Passkey (P-256) 的优势!
```
...
def scalar_mult(k, G):
    ...
    for i in range(256):  # 256 次循环
        if k & (1 << i):
            result = point_add(result, G)  # 点加法
        G = point_double(G)                # 点倍乘
    return result

# 每次 point_add/point_double 需要:
# - 多次大数乘法 (256位 × 256位)
# - 多次模逆运算
# - 总计: 数万个 CPU 周期
```
...
硬件优化技术:
1. Montgomery 乘法: 避免除法，用移位代替
2. 投影坐标: 避免模逆运算
3. 预计算表: 存储 2^i × G 加速查表
4. 流水线: 多个运算并行执行
```
...
| 算法           | 软件 (单核)  | 硬件加速               | 提升倍数       |
|--------------|----------|--------------------|------------|
| SHA-256      | 200 MB/s | 2 GB/s (Intel SHA) | 10x        |
| SHA-256 (挖矿) | 50 MH/s  | 110 TH/s (ASIC)    | 2,000,000x |
| secp256r1 签名 | 20,000/s | 100,000/s (SE)     | 5x         |
| secp256k1 签名 | 20,000/s | 30,000/s (软件优化)    | 1.5x       |
| Ed25519 签名   | 50,000/s | 80,000/s           | 1.6x       |

### 11.7 本项目的硬件加速应用

```
Passkey Wallet 使用 secp256r1 的优势:

用户设备                              链上
┌─────────────────────┐              ┌──────────────────┐
│ Secure Enclave      │              │ EVM              │
│ ┌─────────────────┐ │              │                  │
│ │ 硬件 P-256 加速   │ │   签名       │ P256VERIFY       │
│ │ - 签名 < 1ms     │ │ ──────────→  │ (预编译合约)      │
│ │ - 私钥硬件保护    │ │              │ Gas: 3,450-6,900 │
│ └─────────────────┘ │              │                  │
└─────────────────────┘              └──────────────────┘

完整优势:
1. 签名端: 利用 Secure Enclave 硬件加速，< 1ms 完成
2. 私钥保护: 硬件级别安全，私钥永不导出
3. 验证端: EVM 预编译合约，比 Solidity 实现便宜 100 倍
4. 用户体验: 指纹/Face ID 触发，无需管理助记词
```
